  ...
  interface NoteSlotData {
      string: number,
      beat: any,
      note?: any // Can be null if there's no note at the selected slot
  }
  ...
  class SelectedNoteController {
    ...
    public toggleNoteSelection(note: any) {
    ...
    public getSelectedNote(): any | null {
    ...
    public getNoteBounds(renderer: any): any | undefined {
      if (this.currentSelectedSlot?.note) {
        ...
        const noteBounds = beats?.[1]?.notes?.find((it: any) => it.note.id === this.currentSelectedSlot?.note?.id);
    ...
    public updateCurrentSelection(): void {
      ...
      if (!this.currentSelectedSlot.note) {
        const noteAtSlot = this.currentSelectedSlot.beat.notes
          .find((note: any) => note.string === this.currentSelectedSlot?.string);
        if (noteAtSlot) {
    ...
    private moveSelectedNoteVertical(getNote: (notes: any[]) => NoteSlotData | undefined) {
      if (!this.currentSelectedSlot) {
        return;
      }
    ...
    private moveSelectedNoteHorizontal(getBeat: (beat: any) => any | null): boolean {
      if (!this.currentSelectedSlot) {
        return false;
      }
      ...
      const nextNote = nextBeat.notes.find((note: any) => note.string === this.currentSelectedSlot?.string);
